...
# ── كلمات دالة على محتوى تقني/AI مقبول ──────────────────────
TECH_KEYWORDS = [
    # AI / ML
    "ai", "artificial intelligence", "machine learning", "deep learning",
    "neural", "llm", "gpt", "chatgpt", "gemini", "claude", "openai",
    "anthropic", "mistral", "llama", "diffusion", "generative",
    "language model", "nlp", "computer vision", "robotics",
    # تقنية عامة
    "tech", "technology", "software", "hardware", "chip", "semiconductor",
    "cloud", "data", "cybersecurity", "startup", "silicon",
    "smartphone", "app", "platform", "algorithm", "automation",
    "quantum", "blockchain", "metaverse", "vr", "ar",
    "api", "developer", "code", "programming", "model",
    "google", "microsoft", "apple", "meta", "nvidia", "amd",
    "amazon", "aws", "azure", "tesla", "spacex",
    # استثمار تقني
    "venture", "funding", "valuation", "ipo", "billion", "million",
    "investment", "acquisition", "merger",
]


def _is_tech_related(title: str, summary: str) -> bool:
    """
    يتحقق من أن المقالة لها علاقة بالتقنية أو AI.
    مطلوب كلمة تقنية واحدة على الأقل في العنوان أو الملخص.
    """
...
def _clean_html(text: str) -> str:
    """إزالة علامات HTML وتحويل الكيانات إلى نص عادي"""
    if not text:
        return ""
    try:
        ...
        # fallback: إزالة العلامات بالتعبيرات النمطية
    ...
    for source in RSS_SOURCES:
        try:
            ...
            for entry in feed.entries:
                ...
                title   = clean_text(_clean_html(getattr(entry, "title",   "") or ""))
                summary = clean_text(_clean_html(getattr(entry, "summary", "") or ""))
                ...
                if not title:
                    continue
                # ── فلتر تقني: تجاهل الأخبار غير التقنية ──
                if not _is_tech_related(title, summary):
                    continue
